...
export function concursosView(state, params = {}, savedOnly = false) {
  ...
  return {
    ...
    content: `
      ${savedOnly ? stackHeader(title, `${filtered.length} salvos`) : card(`<div class="hero-card__content"><p class="eyebrow">FOCO DA META</p><h2>Encontre o concurso que combina com seu próximo passo.</h2><p>Compare vagas, salários, datas e comece a estudar pelas questões relacionadas.</p>${button('Ver concursos salvos', { route: '/concursos/salvos', iconName: 'Bookmark' })}</div><div class="hero-card__art"><img src="/assets/kad-mascot-study.png" alt="" width="300" height="300" /></div>`, 'hero-card')}
      ${card(`<form class="filter-bar card--padded" data-form="contest-search">
        <div class="field"><label class="sr-only" for="contest-q">Buscar concurso</label><input class="input" id="contest-q" name="q" value="${escapeHtml(params.q ?? '')}" placeholder="Órgão, banca, cargo ou estado" /></div>
        <div class="field"><label class="sr-only" for="contest-status">Situação</label><select class="select" id="contest-status" name="status"><option value="">Todas as situações</option>${statuses.map((status) => `<option value="${status}" ${params.status === status ? 'selected' : ''}>${status[0].toUpperCase()}${status.slice(1)}</option>`).join('')}</select></div>
        <div class="field"><label class="sr-only" for="contest-region">Região</label><select class="select" id="contest-region" name="region"><option value="">Todas as regiões</option>${regions.map((region) => `<option value="${escapeHtml(region)}" ${params.region === region ? 'selected' : ''}>${escapeHtml(region)}</option>`).join('')}</select></div>
...
export function rankingView(state, params = {}) {
  ...
  return {
    ...
    content: `
      ${card(`<div class="hero-card__content"><p class="eyebrow">RANKING KAD</p><h2>Suba no ranking estudando com consistência.</h2><p>Cada acerto soma pontos conforme a dificuldade. Use os filtros para comparar períodos e áreas.</p>${button('Responder questões', { route: '/questoes', iconName: 'TrendingUp' })}</div><div class="hero-card__art"><img src="/assets/kad-mascot-goal.png" alt="" width="300" height="300" /></div>`, 'hero-card')}
...
export function trailsView(state, params = {}) {
  ...
  return {
    ...
    content: `
      ...
      ${card(`<div class="hero-card__content"><p class="eyebrow">TRILHA ATUAL</p><h2>${escapeHtml(selectedPack?.name ?? selectedDiscipline?.name ?? 'Escolha sua trilha')}</h2><p>${trackQuestions.length} questões distribuídas em até dez níveis, sem repetição.</p>${trackQuestions.length ? button('Começar próximo nível', { route: levels ? `/questoes/sessao?${selectedPack ? `packId=${selectedPack.id}` : `discipline=${encodeURIComponent(selectedDiscipline?.name ?? '')}`}&limit=${chunks.find((chunk) => chunk.length)?.length ?? 1}` : '/questoes', iconName: 'Play' }) : ''}</div><div class="hero-card__art"><img src="/assets/kad-mascot-goal.png" alt="" width="300" height="300" /></div>`, 'hero-card')}
