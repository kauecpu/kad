    ...
    return {
      ...
      content: `
        ...
        <div class="essay-layout">
          ${card(`<div class="essay-editor"><div class="toolbar"><p class="eyebrow">SUA REDAÇÃO</p><span class="muted" data-word-count>${wordCount} palavras</span></div><label class="sr-only" for="essay-content">Texto da redação</label><textarea id="essay-content" class="textarea" data-essay-input data-topic-id="${topic.id}" placeholder="Comece a escrever aqui...">${escapeHtml(draft.content)}</textarea><div class="study-controls"><span class="muted">O rascunho é salvo automaticamente neste navegador.</span>${button('Concluir prática', { route: `/redacao?topic=${topic.id}&stage=review`, iconName: 'CheckCircle2' })}</div></div>`)}
...
export function libraryView() {
  return {
    ...
    content: `
      ...
      ${card(`<div class="detail-panel"><div class="contest-card__heading"><span class="empty-state__icon">${icon('Library')}</span>${badge('Em breve', 'warning', 'Clock3')}</div><h2>Seus materiais ficarão reunidos aqui.</h2><p class="muted">A estrutura já está preparada para audiobooks, anotações e flashcards. A publicação desses conteúdos será integrada em uma etapa posterior.</p></div>`)}
      <div class="action-grid"><div class="action-card"><span class="action-card__icon">${icon('Headphones')}</span><div><h3>Audiobooks</h3><p>Escute conteúdos enquanto acompanha os capítulos.</p></div></div><div class="action-card"><span class="action-card__icon">${icon('StickyNote')}</span><div><h3>Anotações</h3><p>Registre ideias vinculadas ao material estudado.</p></div></div><div class="action-card"><span class="action-card__icon">${icon('Layers3')}</span><div><h3>Flashcards</h3><p>Revise conceitos em sessões curtas.</p></div></div></div>
    `,
  };
}
...
export function performanceView(state) {
  ...
  return {
    ...
    content: `
      ...
      <div class="summary-grid">${card(stat(String(performance.total), 'Questões respondidas', 'BookOpen'))}${card(stat(String(performance.correct), 'Acertos', 'CheckCircle2', 'success'))}${card(stat(String(performance.wrong), 'Erros', 'XCircle', 'danger'))}${card(stat(String(state.favorites.length), 'Favoritas', 'Bookmark', 'warning'))}</div>
