  ...
  return {
    ...
    content: `
      ${workspaceHero({
        ...
        imageSrc: '/assets/kad-mascot-writing.png',
      })}
...
export function libraryView(): ViewModel {
  return {
    ...
    content: `
      ...
      <section class="notice-panel" aria-labelledby="library-status"><div class="notice-panel__heading"><span class="empty-state__icon">${icon('Library')}</span>${badge('Em breve', 'warning', 'Clock3')}</div><h2 id="library-status">Seus materiais ficarão reunidos aqui.</h2><p>A estrutura já está preparada para audiobooks, anotações e flashcards. A publicação desses conteúdos será integrada em uma etapa posterior.</p></section>
