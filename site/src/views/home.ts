...
export function homeView(state: SiteState): ViewModel {
  ...
  const focusCard = focus
    ? card(`<div class="detail-panel"><div class="contest-card__heading"><span class="contest-mark">${escapeHtml(focus.shortName)}</span><div class="question-meta">${savedFocus ? '' : badge('Sugestão', 'accent')}${badge(focus.status === 'aberto' ? 'Inscrições abertas' : 'Acompanhar', focus.status === 'aberto' ? 'success' : 'warning')}</div></div><div><h3>${escapeHtml(focus.title)}</h3><p class="contest-card__organ">${escapeHtml(focus.organ)} · ${escapeHtml(focus.state)}</p></div><div class="detail-row"><span>Vagas</span><strong>${focus.vacancies.toLocaleString('pt-BR')}</strong></div><div class="detail-row"><span>Salário até</span><strong>${formatCurrency(focus.salaryMax)}</strong></div>${button(savedFocus ? 'Ver minha meta' : 'Conhecer concurso', { route: `/concursos/${focus.id}`, variant: 'secondary', className: 'full-width' })}</div>`)
    ...
    content: `
      ...
      <div class="dashboard-grid">
        ...
        <aside class="dashboard-aside" aria-label="Resumo complementar">
          ${performance.total ? section('Seu ritmo', card(`<div class="detail-panel"><div class="toolbar"><strong>${weeklyAnswered} questões</strong><span class="muted">últimos 7 dias</span></div>${progress(weeklyProgress, `Meta semanal: ${weeklyAnswered} de ${state.preferences.weeklyGoal}`)}<p class="muted">${weeklyProgress >= 100 ? 'Meta concluída. Excelente constância!' : `Faltam ${Math.max(0, state.preferences.weeklyGoal - weeklyAnswered)} para concluir a meta.`}</p>${button('Ajustar meta', { route: '/meta', variant: 'ghost', size: 'sm' })}</div>`)) : ''}
          ${section(savedFocus ? 'Minha meta' : focus ? 'Concurso recomendado' : 'Defina sua meta', focusCard)}
        </aside>
      </div>`,
  };
}
