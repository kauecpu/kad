...
export default function FlashcardsScreen() {
  ...
  const visibleCards = useMemo(() => flashcards.filter({ query, deckId: deckFilter, tag: '', state: 'active' }), [deckFilter, flashcards, query]);
      ...
      <ScrollView
        ...
        {reviewing ? (
          ...
          <>
            ...
            <Section title={query || deckFilter !== 'all' ? 'Cards encontrados' : 'Cards ativos'}>
              {flashcards.hydrated ? visibleCards.length ? visibleCards.map((card) => <FlashcardRow key={card.id} card={card} deck={flashcards.decks.find((item) => item.id === card.deckId)} onEdit={() => setEditor({ kind: 'card', card })} onArchive={() => flashcards.archiveCard(card.id)} onDelete={() => Alert.alert('Excluir flashcard?', 'O card será removido desta conta.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Excluir', style: 'destructive', onPress: () => flashcards.deleteCard(card.id) }])} colors={colors} />) : <EmptyState title="Nenhum card encontrado" text="Tente outro termo ou crie um novo card." action="Criar flashcard" onPress={() => setEditor({ kind: 'card' })} /> : <Card style={styles.loading}><ActivityIndicator color={colors.insight} /><Text style={[styles.loadingText, { color: colors.textMuted }]}>Carregando seus cards…</Text></Card>}
            </Section>
          </>
        )}
      </ScrollView>
